...
TEST(BDMInterfaceTest, Vector3DInterface) {
  ...
  EXPECT_FLOAT_EQ(1.2, v3Dinterf.coord_[1]);
  ...
  EXPECT_FLOAT_EQ(1.2, cont_interf_data.oxygen_level_gradient_[8].coord_[1]);

  DiscontinuousInterfaceData disc_interf_data;
  disc_interf_data.ecm_density_gradient_ = v3Dinterf;
  EXPECT_FLOAT_EQ(1.2, disc_interf_data.ecm_density_gradient_.coord_[1]);
