...
BDM_SIM_CLASS(ContainerTestClass, SimulationObject) {
  BDM_CLASS_HEADER(ContainerTestClassExt, 1, dm1_, dm2_);
public:
...
BDM_SIM_CLASS(Neuron, Cell) {
 ...
 public:
  ...
  void DivideImpl(TMostDerived<Scalar> * daughter, double volume_ratio,
    ...
    daughter->neurites_[kIdx].push_back(Neurite(987));
...
BDM_SIM_CLASS(TestObject, SimulationObject) {
  BDM_CLASS_HEADER(TestObjectExt, 1, id_);
